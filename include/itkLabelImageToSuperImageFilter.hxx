 ...
 *
 *=========================================================================*/
#ifndef itkLabelImageToSuperImageFilter_hxx
#define itkLabelImageToSuperImageFilter_hxx

#include "itkLabelImageToSuperImageFilter.h"
...
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::LabelImageToSuperImageFilter()
{
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::EnlargeOutputRequestedRegion(DataObject *)
{
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::BeforeThreadedGenerateData()
{
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::ThreadedGenerateData(
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::AfterThreadedGenerateData()
{
  ...
  for (ThreadIdType i = 1; i < this->GetNumberOfWorkUnits(); ++i)
  {
    ...
    {
      SuperPixelType * superPixel = it.GetSuperPixel();
      if (output->HasLabel(superPixel->GetLabel()))
      {
        // merge the lines in the output's object
        SuperPixelType *lo = output->GetSuperPixel(superPixel->GetLabel());
        typename SuperPixelType::ConstLineIterator lit(superPixel);
        while (!lit.IsAtEnd())
        {
      ...
      else
      {
        ...
        output->AddSuperPixel(superPixel);
      }
    }
  }
...
template <typename TInputImage, typename TOutputImage>
void
LabelImageToSuperImageFilter<TInputImage, TOutputImage>::PrintSelf(std::ostream & os, Indent indent) const
{
